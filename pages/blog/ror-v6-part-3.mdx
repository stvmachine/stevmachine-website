...
To get started with static pages, we’ll first generate a controller using Rails generator that was being used behind the hood with `scaffolding`. Since we’ll be making a controller to handle static pages, we’ll call it the Static Pages controller. We’ll also plan to make actions for a _Home_, _Help_ and _About_ page, designated by the lower-case action names _home_, _help_, and _about_. The generate script takes an optional list of actions, so we’ll include actions for the Home and Help pages directly on the command line, while intentionally leaving off the action for the About page so that we can see how to add it later.
...
maps requests for the URL `/static_pages/home` to the `home` action in the `Static Pages controller`. Even if the action looks like it does nothing, due his inherantance of `ApplicationController` renders a page with the same name. As, we can see if not always necessary to use REST.
...
The addition of this routes will automatically creates a helper called `static_pages_about_url`. The is going to keep failing due we are missing:
...
This test is going to fail, because actually the pages doesn't include that info:
...
> The extension `.erb` means Embedded Ruby (ERb)
...
The content inside of the tags `<title />` includes `<%= yield(:title) %>` that is embeded ruby code that is going to evaluate the variable `:title`. To make this works, we need that each view initializes that variable:
...
- [Learn Enough - Static Pages](learn-enough-static-pages)
- [RSpec][rspec]
