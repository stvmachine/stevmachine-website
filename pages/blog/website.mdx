...
const handleScroll = () => {
  ...
  // menuItems store the Y-position of each section. So, topSection store the position until the second section.
  // menuItems can be recalculated if the screen dimensions change, so it uses a percentage of the distance between sections
